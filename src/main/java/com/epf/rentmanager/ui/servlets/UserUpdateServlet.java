...
public class UserUpdateServlet extends HttpServlet{
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            request.setAttribute("prenom", clientService.findByIdClient(id).getPrenom());
            request.setAttribute("email", clientService.findById(id).getEmail());
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("dd/MM/yyyy");
            request.setAttribute("birth_date", clientService.findById(id).getNaissance().format(formatter));
        ...
        if(request.getParameter("verify")!=null){
            if(request.getParameter("verify").equals("false")){
                ...
                request.setAttribute("birth_date", liste_client.get(0).getBirth_date());
